...
export default function TablePrinted() {
  ...
  const totalRows = data.routePrintedListItems.length;
  const totalPages = Math.ceil(data.routePrintedListItems.length / ROWS_PER_PAGE);
      ...
      <div className="container p-2">
        <div className="rounded-lg bg-gray-800 p-2">
          <div className="mb-4 rounded-t-lg bg-gradient-to-r from-purple-500 to-blue-500 py-2 text-center text-white">
            <h1 className="text-2xl font-medium">Etiquetas impressas</h1>
          </div>
          <table className="table-fixed text-center bg-white overflow-hidden w-full flex-1">
            ...
            <tbody>
            {data.routePrintedListItems.slice(indexOfFirstRow, indexOfLastRow).map((row, index) => (
